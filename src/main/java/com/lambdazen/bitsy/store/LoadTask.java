        ...
        for (int i=0; i < inputs.length; i++) {
            ...
            try {
                ...
                while ((rec = recordReader.next()) != null) {
                    ...
                    // Logic to commit
                    if (isTxLog) {
                        // Commit only on Tx records. This is to ensure that the last bad Tx (if one exists) doesn't get loaded
                    ...
                    case L:
                        ...
                        if (isTxLog) {
                            throw new BitsyException(BitsyErrorCodes.INTERNAL_ERROR, "Found an unexpected log (L) record in file " + fileName + " at line " + lineNo);
                        }
                        ...
                        if (Long.parseLong(rec.getJson()) == lastTxLogNumber) {
                            ...
                            if (recordReader.next() == null) {
                                ...
                                throw new BitsyException(BitsyErrorCodes.INCOMPLETE_TX_FLUSH, "File " + inputLog + " has an L record in line " + lineNo + " with the last valid Tx log number " + lastTxLogNumber);                            
                            }
                        }
                        break;

                    case T:
                        throw new BitsyException(BitsyErrorCodes.INTERNAL_ERROR, "Found an unexpected transaction (T) record in file " + fileName + " at line " + lineNo);
                    ...
                    default:
                        throw new BitsyException(BitsyErrorCodes.INTERNAL_ERROR, "Unhandled record type " + rec.getType() + " in file " + fileName + " at line " + lineNo);
                    }
                }
                ...
                if (isTxLog) {
                    ...
                    if ((vCount > 0) || (eCount > 0)) {
                        ...
                        log.warn("Throwing away {} vertices and {} edges that were not successfully committed", vCount, eCount);
                ...
                if (usingSerialLoader && (e instanceof BitsyException) && (((BitsyException)e).getErrorCode() == BitsyErrorCodes.INCOMPLETE_TX_FLUSH)) {
                    // Fix the V/E log
                    ...
                    toThrow = new BitsyException(BitsyErrorCodes.DATABASE_IS_CORRUPT, "The database files are corrupt. Please restore a backup version", e);
