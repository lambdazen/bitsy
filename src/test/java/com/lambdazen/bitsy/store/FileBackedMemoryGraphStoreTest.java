...
public class FileBackedMemoryGraphStoreTest extends FileBasedTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        this.store = new FileBackedMemoryGraphStore(new MemoryGraphStore(true), tempDir("test-fbmgst"), 2*1024, 1); // 2K buffer to test rollover
...
//    }
    ...
    public void testVertexInserts() {
        ...
        System.out.println("Took " + (System.currentTimeMillis() - ts) + " ms to commit " + (numPerCommit * numCommits) + " vertices"); 
    ...
    public void testBipartiteGraph() {
        for (boolean reverse : new boolean[] {false, true}) {
            for (boolean useEdgeLabels : new boolean[] {true, false}) {
                System.out.println("Entering iteration with reverse " + reverse + ", and useEdgeLabels " + useEdgeLabels);
                ...
                for (int i=0; i < outVertices.length; i++) {
                    ...
                    if (i % numPerCommit == 0) {
                        ...
                        //System.out.println("Took " + duration + " ms to save " + size + " edges. Rate = " + (duration / size) + " ms per edge");
                ...
                for (int i=0; i < outVertices.length; i++) {
                    ...
                    if (useEdgeLabels && Math.random() < 0.2) {
                        ...
                        edgeList = store.getEdges(outVertices[i], reverse ? Direction.IN : Direction.OUT, new String[] {"Label 0", "Label 1", "Label 2"}); 
                    } else {
                    ...
                    for (int j=0; j < 3; j++) {
                        ...
                        assertTrue(edge.getProperties() == null || edge.getProperties().get("foo").equals("bar"));
                        ...
                        for (int k=0; k < 3; k++) {
                            ...
                            if (otherVertexId.equals(ins[k])) {
                                ...
                                if (useEdgeLabels) {
                                    ...
                                    if (Math.random() < 0.2) {
                                        List<EdgeBean> edgeByLabelList = store.getEdges(outVertices[i], reverse ? Direction.IN : Direction.OUT, new String[] {label});
                                        assertEquals(1, edgeByLabelList.size());
                                        assertEquals(edge.getId(), edgeByLabelList.get(0).getId());
                                    }
                                } else {
                ...
                double duration = (System.currentTimeMillis() - ts);
                System.out.println("Took " + duration + " ms to save " + (numCommits * numPerCommit * 3) + " edges. Rate = " + (duration / (numCommits * numPerCommit * 3)) + " ms per edge");
                ...
                System.out.println("Took " + duration + " ms to delete " + inVertices.length + " vertices with 3 incident edges. Rate = " + (duration / inVertices.length) + " ms per vertex");

                // Check if there are no edges into inVertices
                ts = System.currentTimeMillis();
                for (int i=0; i < outVertices.length; i++) {
                    List<EdgeBean> edgeList = store.getEdges(outVertices[i], reverse ? Direction.IN : Direction.OUT, null);
        ...
        public Collection<BitsyEdge> getEdgeChanges() {
            if (edges == null) {
                ...
                while (outIter.hasNext()) {
                    ...
                    BitsyEdge edge = new BitsyEdge(edgeUUID, DictionaryFactory.fromMap(edgeProps), null, BitsyState.M, Integer.MIN_VALUE, label, outV, inV);
                    edges.add(edge);
                    ...
                    if (Math.random() < 0.01) {
                        ...
                        edges.add(new BitsyEdge(edgeUUID, null, null, BitsyState.U, Integer.MIN_VALUE, label, outV, inV));
                    }
                    ...
                    if (Math.random() < 0.05) {
                        ...
                        edges.add(new BitsyEdge(deleteEdgeId, DictionaryFactory.fromMap(edgeProps), null, BitsyState.M, 1, "To be removed", outV, inV));

                        // Now remove it
                        edges.add(new BitsyEdge(deleteEdgeId, DictionaryFactory.fromMap(edgeProps), null, BitsyState.D, 2, "To be removed", outV, inV));
                    }
                }
